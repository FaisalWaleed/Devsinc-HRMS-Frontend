      ...
      <Grid key={index} container>
                     ...
                     inputProps={{
                       ...
                       onChange: (event) => {
                         if(event.target.value){
                           resetField('ticket_form',`${option}.title`,null)
        ...
        {ticketOptionsChosen[index].department_id !== 0 && ticketOptionsChosen[index].department_id ?
          <ItemGrid xs={3} sm={3} md={3}>
            <Field name={`${option}.title`} component={({input}) => (
              <CustomInput
                isSelect={true}
                formControlProps={{
                  fullWidth: true
        ...
        {(ticketOptionsChosen[index].department_id !== 0 && ticketOptionsChosen[index].title !== 0 && ticketOptionsChosen[index].title != null) ?
          <ItemGrid xs={4} sm={4} md={4}>
            <Field name={`${option}.user_id`} component={({input}) => {
                    ...
                    <div className={classes.chips}>
                      {selected.indexOf(0) !== -1
                        ...
                        : selected.map(value => <Chip key={value} label={
                          ticketOptions[ticketOptionsChosen[index].department_id].titles[ticketOptionsChosen[index].title].users[ticketOptions[ticketOptionsChosen[index].department_id].titles[ticketOptionsChosen[index].title].users.findIndex(user => user.id === value)].first_name + " " +
                          ticketOptions[ticketOptionsChosen[index].department_id].titles[ticketOptionsChosen[index].title].users[ticketOptions[ticketOptionsChosen[index].department_id].titles[ticketOptionsChosen[index].title].users.findIndex(user => user.id === value)].last_name } className={classes.chip}/>)}
                    </div>
                  ),
                  multiple: true,
                  value: input.value,
              ...
              >
                ...
                {
                  ticketOptions[ticketOptionsChosen[index].department_id] ?
                    ticketOptions[ticketOptionsChosen[index].department_id].titles[ticketOptionsChosen[index].title] ?
                      ticketOptions[ticketOptionsChosen[index].department_id].titles[ticketOptionsChosen[index].title].users.map((user,index) => (
                          <MenuItem
                            key={index}
                            value={user.id}
                          >
